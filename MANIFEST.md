...
| .devtools/ (submodule) | Shared makefiles, workflows, hooks — ostara-labs/devtools @ v1.12.1 |
...
| typescript/svelte.config.js | SvelteKit config; adapter-node (prod target Railway/ECS) |
| typescript/vite.config.ts | Vite config; Tailwind v4 + Paraglide plugins |
| typescript/biome.json | Biome config; lints TS/JS/JSON only — `.svelte` is checked by svelte-check |
| typescript/project.inlang/ | Inlang project: settings.json + paraglide.config.ts (locale strategy, URL patterns) |
| typescript/messages/ | Paraglide translations: fr.json (source of truth), fr.meta.json (translator sidecar), en.json |
| typescript/scripts/check-i18n.ts | i18n completeness gate (spec 27.17): fails if a FR key lacks an EN translation or meta entry |
| typescript/src/routes/ | SvelteKit routes; `[locale]/` segment (FR/EN prefix routing) |
| typescript/src/lib/components/ | UI components: MapShell (MapLibre GL JS shell), LanguageSwitcher |
| typescript/src/lib/i18n/ | i18n check logic + vitest tests (27.17 gate, messages smoke test) |
| typescript/src/lib/paraglide/ | Generated by Paraglide compiler (gitignored) |
